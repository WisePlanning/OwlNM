...
void sensor_event(struct bufferevent *bev, short events, void *ctx)
{
	...
	else if (events & BEV_EVENT_ERROR)
	{
		...
		do
		{
			...
			if (server->sensor1_fd != 0)
				close(server->sensor1_fd);
			if (server->sensor2_fd != 0)
				close(server->sensor2_fd);
		...
		} while (retry < 10 && (!(server->sensor1_fd > 0) || !(server->sensor2_fd > 0) || server->sensor1_fd == server->sensor2_fd));

		if (server->sensor1_fd < 1 || server->sensor2_fd < 1)
		{
...
// if there is no server address,
	...
	do
	{
		...
		LOG_WRITE("Sensor 2 = %d\n", sensor_device_2);
	} while (retry < 10 && (!(sensor_device_1 > 0) || !(sensor_device_2 > 0) || sensor_device_1 == sensor_device_2));

	if (sensor_device_1 < 1 || sensor_device_2 < 1)
	{
		LOG_WRITE("Could not get Sensors\n");
		exit(EXIT_FAILURE);
	}
